...
const CustomerGrowthChart = ({ data = [], onMonthClick }) => {
  const handleClick = (e) => {
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-2xl p-6 border border-gray-200 dark:border-slate-700 shadow-sm cursor-pointer">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-base font-bold text-gray-900 dark:text-white">Customer Growth</h2>
          <p className="text-sm text-gray-400 dark:text-slate-500 mt-0.5">New customers per month (Click to drill down)</p>
        </div>
      </div>

      <ResponsiveContainer width="100%" height={260}>
        <BarChart data={data} onClick={handleClick} margin={{ top: 5, right: 5, bottom: 0, left: 0 }}>
